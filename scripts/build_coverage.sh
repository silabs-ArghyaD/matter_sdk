...
lcov --capture --directory "$OUTPUT_ROOT/obj/src" \
    --ignore-errors format,unsupported,inconsistent,mismatch \
    --exclude="$PWD"/zzz_generated/* \
    --exclude="$PWD"/third_party/* \
    --exclude=/usr/include/* \
    --output-file "$COVERAGE_ROOT/lcov_test.info"

lcov --ignore-errors format,unsupported,inconsistent,mismatch \
...
genhtml "$COVERAGE_ROOT/lcov_final.info" \
    --ignore-errors inconsistent,category,count,mismatch \
